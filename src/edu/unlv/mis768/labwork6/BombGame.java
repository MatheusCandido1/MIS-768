...
public class BombGame {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i = 0; i < cityList.size(); i++){
			// print the distance
			System.out.println("City "+(i+1)+" is "+cityList.get(i).getDistance(bomb)+" units aways from the bomb");
			// if the distance is less than or equal to the radius
			if (cityList.get(i).getDistance(bomb) <= radius){
				// show affected
	 ...
	 * @throws IOException
	 */
	public static ArrayList<Point> readCities(ArrayList<Point> cityList) throws IOException{
